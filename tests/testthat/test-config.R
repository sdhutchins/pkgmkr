...
"
  ...
  # Check that package was created with essential files
  pkg_path <- file.path(tmp_dir, "minimalpkg")
  ...
  # README.md creation may fail in some environments, so we check if it exists
  # but don't fail the test if it doesn't (it's created with tryCatch)
