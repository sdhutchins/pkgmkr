...
test_that("mk_pkg handles author names with multiple parts", {
  ...
  # Read DESCRIPTION file as a single string to handle multi-line Authors@R field
  desc_path <- file.path(pkg_path, "DESCRIPTION")
  desc_content <- paste(readLines(desc_path), collapse = "\n")

  # Check that the author information is present
  expect_match(desc_content, "Mary")
  expect_match(desc_content, "Jane Smith Wilson")
  expect_match(desc_content, "mary@example.com")
})
...
test_that("mk_pkg validates email format when provided", {
  tmp_dir <- withr::local_tempdir()

  expect_error(
    mk_pkg(
      path = file.path(tmp_dir, "testpkg1"),
      author = "Test Author",
      email = "not-an-email"
    ),
    "valid email address"
  ...
  expect_error(
    mk_pkg(
      path = file.path(tmp_dir, "testpkg2"),
      author = "Test Author",
      email = "@example.com"
    ),
    "valid email address"
...
test_that("mk_pkg works without email", {
  ...
  tmp_dir <- withr::local_tempdir()
  pkg_path <- file.path(tmp_dir, "noemail")
